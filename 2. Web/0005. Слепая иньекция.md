...
1. Параметр `number` вставляется прямо в SQL-запрос без проверки.
2. Сервер возвращает только `true` или `false` в зависимости от того, есть ли записи.
3. Нужно извлечь флаг из таблицы `flag`.
